...
Система принимает корпоративные документы. Поддерживаемые форматы различаются по этапу реализации:

**MVP (реализовано):**

| Формат | Библиотека | Что извлекается | Ограничение |
|---|---|---|---|
| Markdown (`.md`) | прямой read | Текст целиком; секции по `#`/`##`/`###` | — |
| PDF текстовый (`.pdf`) | PyMuPDF | Нативный текстовый слой постранично | Только born-digital PDF; скан → пустой текст |

**Scale (запланировано, не реализовано в MVP):**

| Формат | Библиотека | Примечание |
|---|---|---|
| PDF сканированный | PyMuPDF + easyocr | OCR ru+en, 300 DPI |
| PDF с чертежами | PyMuPDF + Qwen2.5-VL | Vision-описание при `VISION_ENABLED=true` |
| DOCX | python-docx | Основной текст, стили |
| XLSX | openpyxl | Значения ячеек |
| CSV | pandas | Очистка BOM и пустых строк |

**Метаданные и уровень доступа** указываются оператором вручную — автоматическая классификация `access_level` по содержимому не реализована и запланирована на Scale-этап.

### Два пути ingestion (MVP)

| Путь | Как запустить | Формат | Кто может |
|---|---|---|---|
| **Batch corpus** | `POST /ingest` без тела | Файлы `.md` / `.pdf` из `docs/corpus/` + `corpus_manifest.json` | manager, admin |
| **Single-doc** | `POST /ingest` с JSON-телом | Markdown-текст строкой (`content`); `doc_id` и `access_level` обязательны | manager, admin |

> **Важно:** single-doc path принимает **текстовое содержимое**, не бинарный файл. Загрузка файла через браузер (multipart upload) — Scale-этап.
...
### Таблица `audit_log` — ✅ реализовано в MVP (SQLite), Scale (PostgreSQL)

**Реализована в MVP** через SQLite WAL: `backend/db/audit_store.py`. Доступна через `GET /audit-log` (роль admin).

Audit logging для MVP обеспечивается тремя слоями: SQLite audit store (каждый запрос), Langfuse traces (трассировка нод агента) и `knowledge_gaps` (неотвеченные запросы).

**Целевая схема для Scale (PostgreSQL):**

```sql
-- Целевая схема Scale (MVP использует SQLite с аналогичной структурой)
