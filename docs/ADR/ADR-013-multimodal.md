...
|---|---|---|---|
| **Слой 0: pymupdf (born-digital PDF + Markdown)** | Извлечение текста из нативных PDF и Markdown-файлов | MVP — обязательно | ✅ Реализовано (`backend/ingestion/document_processor.py`) |
| **Слой 1: pymupdf + easyocr** | OCR отсканированных документов, парсинг таблиц сложной вёрстки | Should Have | 🔜 Не реализовано в MVP — Scale-этап |
| **Слой 2: Qwen2.5-VL (vision)** | Описание чертежей и изображений через vision-модель | Should Have | 🔜 Добавляется поверх Слоя 1 — Scale-этап |
| **Scale** | Azure Document Intelligence / Tesseract 5 для промышленных сканов; DOCX, XLSX, CSV; коннекторы к Confluence и GitLab | Scale | 🔜 Вне скоупа MVP |
